...
class PostIntegrationTest {
    ...
    void 게시글_작성_목록_상세_삭제_성공_흐름() throws Exception {
        ...
        assertEquals("첫 게시글", listBody.get("data").get(0).get("title").asText());
    ...
    void 게시글_목록은_최신순_정렬() throws Exception {
        ...
        JsonNode listBody = objectMapper.readTree(listResult.getResponse().getContentAsString());
        assertEquals("최신 글", listBody.get("data").get(0).get("title").asText());
        assertEquals("오래된 글", listBody.get("data").get(1).get("title").asText());
    }
